...
struct State {
    ...
    int move(int dir) { // moveCube paso a ser move para evitar colision de nombre
                        // y solo recibe una direccion norte, sur... para usar la matriz de orientaciones
        int bottom = packedori & 7;
...
int moveCube(int packedori, int dir) { // saqué la funcion a fuera del struct
        int bottom = packedori & 7;       // para que tenga el parametro de la empaquetación completa
        int front = (packedori >> 3) & 7; // para realizar movimientos en el cubo
...
void calculateOrientations() {  // el bfs
...
int main() {
    calculateOrientations(); //inicializa las orientaciones
    int T, R, C, A, B;
    cin >> T;
